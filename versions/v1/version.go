...
func (v *Version) Rollback(deployHosts []hosts.GHost, dirName string) error {
	...
	for _, ghost := range deployHosts {
		...
		releaseLink := v.safeShell.lnSfnShellString(sshRepoDir, v.conf)
		_, err = ghost.Command(releaseLink)
		if err != nil {
			logger.Warning(err)
			continue
		}
		hookPostHost := v.safeShell.CdMultipleShell(v.conf.DstDir, v.conf.HookPostHost)
...
func (v *Version) Run(workerPath string, deployHosts []hosts.GHost) error {
	...
	hookPostServer := v.safeShell.CdMultipleShell(workerPath, v.conf.HookPostServer)
	logger.InfoF("Execute commands in the project `%s`", hookPostServer)
	...
	//开始进行打包
	tarCommand := v.safeShell.CdTarShellString(workerPath, tarGzFile, v.conf)
	...
	for _, ghost := range deployHosts {
		...
		_, err := ghost.Command(v.safeShell.deployPathExistShellString(v.conf))
		if err != nil {
		...
		//清理过期文件
		cleanPathFileCmd := v.safeShell.CleanDstRepoFiles(v.conf)
		...
		unTar := v.safeShell.CdUnTarShellString(tarGzFile, sshRepoDir, v.conf)
		...
		hookPreHost := v.safeShell.CdMultipleShell(sshRepoDir, v.conf.HookPreHost)
